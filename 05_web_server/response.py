...
def get_handler(*,
    ...
    if request_parser.is_file():
        try:
            body_content = doc_root_helper.get_file_content(
                request_parser.get_path()
            )
            headers['Content-Type'] = EXTENSION_TO_CTYPE[
                doc_root_helper.extension(request_parser.get_path())
            ]
        except FileNotFound:
            status = 404
        ...
        try:
            body_content = doc_root_helper.get_dir_index_file_content(
                request_parser.get_path()
            )
        except DirectoryNotFound:
            status = 404
            body_content = f'index.html does not exist in' \
                f' {request_parser.path} directory'
...
def head_handler(*,
                 ...
                 doc_root_helper=None,
                 **kwargs):
    response, headers = get_handler(method=method,
